...
switch (toLower _action) do
{
	case "money":
	{
		if (_value > 0) then
		{
			...
			//diag_log format ["%1, %2", name player, _value];
			player globalChat format ["[NOTICE] %1 used the admin menu to obtain $%2", name player, _value];
		};
	};
	case "teleport":
	{
		...
		player globalChat format['[NOTICE] %1 used the admin menu to teleport to "%2"', profileName, _value];
	};
	case "vehicle":
	{
	...
	case "godmode":
	{
		player globalChat format ["[NOTICE] %1 used the admin menu to obtain godmode", name player];
	};
	case "object":
	{
		player globalChat format ["[NOTICE] %1 used the admin menu to obtain %2", name player, _value];
	...
	case "playermarkers":
	{
		player globalChat format ["[NOTICE] %1 used the admin menu to obtain player markers", name player];
	};
};
...
if (!isNil "_cfg" && {isClass _cfg}) then
{
	...
	player globalChat format['[NOTICE] %1 used the admin menu to obtain a "%2"', profileName, _displayStr];
};
